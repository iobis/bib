...
counts <- dbGetQuery(con, "select country, count(*) from imis.wos group by country order by count(*) desc", stringsAsFactors=FALSE)
...
ggplot() + geom_bar(data=counts, aes(x="", y=count, fill=country), stat="identity", width=3) +
  ...
  scale_fill_manual(values=c(colorRampPalette(brewer.pal(12, "Paired"))(ncolors), rep("#cccccc", nrow(counts) - ncolors))) +
  xlab("") +
  ylab("") +
...
aff <- dbGetQuery(con, "select * from imis.wos", stringsAsFactors=FALSE)
...
for (ref in refs) {
  cdata <- aff %>% filter(brefid == ref) %>% distinct(country)
  if (nrow(cdata) > 1) {
